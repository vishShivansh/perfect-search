...
export default function SmartSearch() {
    ...
    const getIcon = (type) => {
        const icons = {
            file: <PaperclipIcon className="w-4 h-4 text-gray-500" />,
            video: <Video className="w-4 h-4 text-gray-500" />,
            image: <ImageIcon className="w-4 h-4 text-gray-500" />,
            folder: <Folder className="w-4 h-4 text-gray-500" />,
    ...
    const categoryIcons = {
        Files: <PaperclipIcon className="w-4 h-4 inline-block mr-1" />,
    ...
    return (
        <div className={`relative mx-auto mt-6 transition-all duration-300 w-full max-w-sm sm:max-w-md ${tabs.length > 3 ? "md:max-w-xl" : "md:max-w-sm"} px-2 sm:px-0`}>
            <div className="w-full bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div className={`transition-all duration-500 ease-in-out ${results.length > 0 || query ? "max-h-[60vh] opacity-100" : "max-h-0 opacity-0"}`}>
                    <div className="flex flex-wrap justify-between items-start sm:items-center border-b-2 border-gray-200 text-sm sm:text-base">
                        ...
                        {/* Settings Dropdown */}
                        <div ref={settingsRef} className="px-3 sm:px-4 py-1">
                            <button onClick={() => setShowSettings(!showSettings)}>
                                <Settings className="w-4 h-4 sm:w-5 sm:h-5 text-gray-500 cursor-pointer" />
                            </button>

                            <div className={`absolute top-30 sm:top-18 right-0 sm:right-4 bg-white shadow-xl border border-gray-200 z-100 rounded-md text-sm sm:text-base p-2 w-[90vw] sm:w-40 max-w-sm transition-all duration-500 ease-in-out ${showSettings ? "max-h-60 opacity-100 overflow-auto" : "max-h-0 opacity-0 overflow-hidden"}`}>
                                            ...
                                            <div className="flex gap-2 mt-2 sm:mt-0">
                                                ...
                                                >
                                                    ...
                                                    {copiedId === item.id && toast && (
                                                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-black text-white text-xs px-2 py-1 rounded-lg shadow-lg whitespace-nowrap">
                                                            Link copied!
                                                        </div>
                                                    )}
                                                </button>
